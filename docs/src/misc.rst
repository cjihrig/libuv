...
.. c:function:: int uv_if_indextoname(unsigned int ifindex, char* buffer, size_t* size)
    ...
    `*size` indicates the length of `buffer`, which is used to store the result.
    On success, zero is returned, `buffer` contains the interface name, and
    `*size` represents the string length of `buffer`. A negative result is
    returned on error. If `buffer` is not large enough to hold the result,
...
.. c:function:: int uv_if_indextoiid(unsigned int ifindex, char* buffer, size_t* size)
    ...
    represents the size, including the NULL terminator, required to hold the
    result.
